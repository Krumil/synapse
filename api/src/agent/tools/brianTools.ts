...
const brianAgentSchema = z.object({
	...
	chainId: z.number().optional().describe("Chain ID for the transaction (optional)"),
});

export const brianAgentTool = tool(
	async ({ prompt, address, userId, chainId }) => {
		console.log('brianAgentTool', prompt, address, userId, chainId);
		try {
			...
			messages = [
				...
				{
					sender: "brian",
					content: result.data?.description || result.answer || "No response content available"
				}
			];
			...
			if (axios.isAxiosError(error)) {
				...
				messages = [
					...messages,
					{ sender: "user", content: prompt },
					{ sender: "brian", content: errorMessage }
				];
				conversationHistory.set(conversationKey, messages);
