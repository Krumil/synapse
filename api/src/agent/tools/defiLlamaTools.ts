...
async function readChainData(chain: string): Promise<ChainData | null> {
    try {
        ...
        const chainData: ChainData[] = allYields.filter((p) => p.chain.toLowerCase() === chain.toLowerCase());
...
const getTopDefiProtocolsTool = tool(
    async ({ minTvl = 1000000, minApy = 0, limit = 10 }: { minTvl?: number; minApy?: number; limit?: number }) => {
        try {
            ...
            const filteredProtocols = allChainData
                ...
                .sort(
                    (a, b) =>
                        b.protocols.reduce((acc, p) => acc + (p.tvlUsd || 0), 0) -
                        a.protocols.reduce((acc, p) => acc + (p.tvlUsd || 0), 0)
                )
                .slice(0, limit);
            ...
            throw new Error(
                `Failed to get top DeFi protocols: ${error instanceof Error ? error.message : "Unknown error"}`
            );
        }
    },
    {
...
export const defiLlamaTools = [getDefiDataTool, getTopDefiProtocolsTool];
