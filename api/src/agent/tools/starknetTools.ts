...
const getTopStarknetTokensTool = tool(
    async ({ limit = 10 }: { limit?: number }) => {
        try {
            ...
            throw new Error(
                `Failed to fetch Starknet tokens: ${error instanceof Error ? error.message : "Unknown error"}`
            );
        }
    },
    {
    ...
    async ({ address }: { address: string }) => {
        try {
            ...
            const volumeResponse = await fetch(
                `https://starknet.impulse.avnu.fi/v1/tokens/${address}/exchange-volumes`
            );
            ...
            throw new Error(
                `Failed to fetch exchange data: ${error instanceof Error ? error.message : "Unknown error"}`
            );
        }
    },
    {
...
const getTokenPriceFeedTool = tool(
    async ({ address }: { address: string }) => {
        try {
            ...
            const previousPrice = priceFeed.find(
                (entry: { date: string; value: number }) => entry.date <= oneDayAgo
            )?.value;

            // Calculate 24h price change percentage
            const priceChange24h = previousPrice ? ((currentPrice - previousPrice) / previousPrice) * 100 : null;
            ...
            throw new Error(
                `Failed to fetch token price feed: ${error instanceof Error ? error.message : "Unknown error"}`
            );
        }
    },
    {
        ...
        description:
            "Get current price and 24h price change for a token on Starknet, along with recent historical prices",
        schema: z.object({
            address: z.string().describe("The token's contract address on Starknet"),
        }),
    }
