...
const createMemecoinTool = tool(
    async ({ owner, name, symbol, initialSupply }) => {
        try {
            ...
            throw new Error(`Failed to create memecoin: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
    },
    {
        name: "create_memecoin",
        description: "Creates a new memecoin on StarkNet and returns the calculated memecoin address",
        schema: z.object({
            ...
            initialSupply: z
                .string()
                .describe("The initial supply of tokens (as a string representing a big integer between 1 and 10^10)"),
        }),
    }
);
...
const launchMemecoinTool = tool(
    async ({
        ...
        try {
            ...
            throw new Error(`Failed to launch memecoin: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
    },
    {
            ...
            teamAllocations: z
                .array(
                    z.object({
                        ...
                        amount: z
                            .string()
                            .describe("The amount of tokens to allocate to the address in units not percentage"),
                    })
                )
                .optional(),
            holdLimit: z
                .string()
                .optional()
                .describe("The hold limit for the memecoin in percentage, suggested: 5% per wallet"),
            antiBotDuration: z
                .string()
                .optional()
                .describe("The duration of the anti-bot period in seconds, suggested: 1 hour (3600 seconds)"),
